use std::hash::Hash;
use spin_bitwise::{ReadLockGuard, WriteLockGuard};
...
pub struct Accessor<'a, K: 'a, V: 'a> {
    table: ReadLockGuard<'a, Table<K, V>>,
    idx: usize
}
...
pub struct MutAccessor<'a, K: 'a, V: 'a> {
    table: WriteLockGuard<'a, Table<K, V>>,
    idx: usize
}

impl <'a, K, V> Accessor<'a, K, V> {
    pub fn new(table: ReadLockGuard<'a, Table<K, V>>, idx: usize) -> Accessor<'a, K, V> {
        Accessor {
            table: table,
            idx: idx
        }
...
impl <'a, K, V> MutAccessor<'a, K, V> {
    pub fn new(table: WriteLockGuard<'a, Table<K, V>>, idx: usize) -> MutAccessor<'a, K, V> {
        MutAccessor {
            table: table,
            idx: idx
        }
